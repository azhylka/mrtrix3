  ...
  if (!opt.empty()) {
    ...
    if (COMs.size() > max_node_index + 1) {
      WARN("Parcellation image \"" + exemplars_input_path.string() +
           ...
           ") than are present in input assignments file \"" + assignments_input_path.string() + "\" (" + str(max_node_index) +
           ")");
    ...
    case 1: // One file per node
      for (std::vector<node_t>::const_iterator i = nodes.begin(); i != nodes.end(); ++i)
        writer.add(*i, output_prefix + str(*i) + ".tck", !weights_prefix.empty() ? (weights_prefix + str(*i) + ".csv") : "");
